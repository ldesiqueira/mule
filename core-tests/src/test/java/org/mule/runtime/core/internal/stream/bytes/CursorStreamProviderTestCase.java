...
public class CursorStreamProviderTestCase extends AbstractByteStreamingTestCase {
  ...
  public CursorStreamProviderTestCase(String name, int dataSize, int bufferSize) {
    super(dataSize);
    this.bufferSize = bufferSize;
    halfDataLength = data.length() / 2;
    if (dataSize <= bufferSize) {
      streamProvider = new BufferedCursorStreamProvider(new ByteArrayInputStream(data.getBytes()), bufferSize, NO_OFF_HEAP);
    } else {
      streamProvider = new BufferedCursorStreamProvider(new ByteArrayInputStream(data.getBytes()), bufferSize, FILE_STORE);
    }
    resetLatches();
  }
