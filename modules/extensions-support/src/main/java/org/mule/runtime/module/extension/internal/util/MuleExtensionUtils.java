...
public class MuleExtensionUtils {
  ...
  public static Message toMessage(Result result, MediaType mediaType, CursorStreamProviderFactory cursorStreamProviderFactory) {
    return Message.builder()
        .payload(asStreamProvider(result.getOutput(), cursorStreamProviderFactory).getValue())
        .mediaType(mediaType)
        .attributes((Attributes) result.getAttributes().orElse(NULL_ATTRIBUTES))
        .build();
  }

  public static <T> Either<CursorStreamProvider, T> asStreamProvider(T value,
                                                                     CursorStreamProviderFactory cursorStreamProviderFactory) {
    if (value instanceof InputStream) {
      return (Either<CursorStreamProvider, T>) cursorStreamProviderFactory.of((InputStream) value);
    } else {
