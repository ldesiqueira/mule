...
import static org.mule.runtime.module.extension.internal.util.MuleExtensionUtils.asStreamProvider;
...
abstract class AbstractReturnDelegate implements ReturnDelegate {
   ...
   * @param cursorStreamProviderFactory the {@link DefaultCursorStreamProviderFactory} to use when a message payload is an {@link InputStream}. Can be {@code null}
  ...
  protected Message toMessage(Object value, ExecutionContextAdapter operationContext) {
    ...
    if (value instanceof Result) {
      return MuleExtensionUtils.toMessage((Result) value, mediaType, cursorStreamProviderFactory);
    } else {
      if (value instanceof Collection && returnsListOfMessages) {
        value = toMessageCollection((Collection<Result>) value, mediaType, cursorStreamProviderFactory);
      }
      return Message.builder()
          .payload(asStreamProvider(value, cursorStreamProviderFactory).getValue())
          .mediaType(mediaType)
